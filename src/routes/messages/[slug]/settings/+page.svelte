...
<div class="mx-7 w-full">
	<div class="flex mt-5 justify-between">
		<div class="flex">
			...
			<div>
				<p class="text-base font-semibold uppercase">Update channel</p>
				<p class="text-xs text-gray-300 mt-2">
					You can also change to private channel by clicking lock icon
				</p>
			</div>
		</div>
		<button
	...
	{#if isPrivate}
		<input
			type="password"
			bind:value={password}
			class="bg-gray-800 mt-3 border text-gray-200 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
	...
	<button
		...
		disabled={loading}
		type="button"
		class="text-white mt-3 bg-gray-800 hover:bg-gray-900 font-medium rounded-full text-sm p-2.5 text-center inline-flex items-center mr-2"
	>
		{#if loading}
			<svg
