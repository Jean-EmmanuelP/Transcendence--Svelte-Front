<script lang="ts">
	...
	onMount(async () => {
		try {
			const tempChannels = await getGroups();
...
<div class="flex mx-7 h-full">
	<div class="flex-1 h-full">
		<p class="mt-5 text-base font-semibold uppercase">Channels - {channels.length}</p>
		<input
			type="text"
			bind:value={name}
		...
		<div class="overflow-y-scroll max-w-full scrollbar-hide">
			{#each channels.filter(e => {
				...
				<ChannelComponent channel={channel} handleLeave={handleLeave}/>
			{/each}
		</div>
	</div>
</div>
